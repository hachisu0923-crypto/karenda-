...
function buildCell(y,m,d,isOther,isToday) {
  ...
  // TODO completion bar
  {
    const hist = _readDailyHist(currentUser?.id || 'anon');
    const rec = hist[key];
    if (rec && rec.total > 0) {
      const pct = Math.round(rec.done / rec.total * 100);
      const barWrap = document.createElement('div');
      barWrap.className = 'day-todo-bar';
      barWrap.title = `TODO ${rec.done}/${rec.total} (${pct}%)`;
      const fill = document.createElement('div');
      fill.className = 'day-todo-fill' + (pct >= 100 ? ' is-full' : pct >= 50 ? ' is-half' : ' is-low');
...
// ──────────────────────────────────────────────────────────────────────────────
// TODO (Sidebar) — Day / Week / Month lists (localStorage)
//  - Day: resets per selected day (default: today)
//  - Week: resets per selected week (Monday start)
//  - Month: resets per selected month
// ──────────────────────────────────────────────────────────────────────────────

const TODO_STORAGE_KEY    = 'todos_v2';
const TODO_DAILY_TMPL_KEY = 'todo_daily_tmpl_v1';  // 毎日繰り返すTODOのテンプレート
const TODO_DAILY_HIST_KEY  = 'todo_daily_hist_v1'; // 日ごとの達成履歴

// ── Daily-template helpers ──

function _readDailyTmpl(userId) {
  try { return JSON.parse(localStorage.getItem(TODO_DAILY_TMPL_KEY) || '{}')[userId] || []; }
  catch(_) { return []; }
}
function _writeDailyTmpl(userId, tmpl) {
  const root = JSON.parse(localStorage.getItem(TODO_DAILY_TMPL_KEY) || '{}');
  root[userId] = tmpl;
  localStorage.setItem(TODO_DAILY_TMPL_KEY, JSON.stringify(root));
}

// ── Daily history helpers ──

function _readDailyHist(userId) {
  try { return JSON.parse(localStorage.getItem(TODO_DAILY_HIST_KEY) || '{}')[userId] || {}; }
  catch(_) { return {}; }
}
function _writeDailyHist(userId, hist) {
  const root = JSON.parse(localStorage.getItem(TODO_DAILY_HIST_KEY) || '{}');
  root[userId] = hist;
  localStorage.setItem(TODO_DAILY_HIST_KEY, JSON.stringify(root));
}

// 指定日の完了状態を履歴に保存（render のたびに呼ばれる）
function _saveDayCompletion(userId, dk, todos) {
  if (!todos.length) return;
  const hist = _readDailyHist(userId);
  hist[dk] = {
    total:   todos.length,
    done:    todos.filter(t => t.done).length,
    doneIds: todos.filter(t => t.done).map(t => t.id)
  };
  _writeDailyHist(userId, hist);
}

// テンプレート + その日の完了状態 を合成して todos 配列を返す
function _getDayTodosForDate(userId, dk) {
  const tmpl   = _readDailyTmpl(userId);
  const record = (_readDailyHist(userId))[dk] || {};
  const doneSet = new Set(record.doneIds || []);
  return tmpl.map(t => ({ ...t, done: doneSet.has(t.id) }));
}
...
function getMonthKeyFromContext(){
  // Follow current calendar month view (curDate)
  return formatYM(curDate);
}

function _readTodoRoot(){
  try{ return JSON.parse(localStorage.getItem(TODO_STORAGE_KEY) || '{}'); }
  catch(_e){ return {}; }
}
function _writeTodoRoot(root){
  localStorage.setItem(TODO_STORAGE_KEY, JSON.stringify(root));
}

function loadTodos(userId, scope, key){
  const root = _readTodoRoot();
  const byUser = root[userId] || {};
  const byScope = byUser[scope] || {};
  const list = byScope[key] || [];
  return Array.isArray(list) ? list.filter(t => t && typeof t.text === 'string') : [];
}

function saveTodos(userId, scope, key, todos){
  const root = _readTodoRoot();
  root[userId] = root[userId] || {};
  root[userId][scope] = root[userId][scope] || {};
  root[userId][scope][key] = todos;
  _writeTodoRoot(root);
}

function pruneTodos(userId, scope, keepKey){
  // Avoid growing forever
  const root = _readTodoRoot();
  const byUser = root[userId];
  if(!byUser || !byUser[scope]) return;
  const byScope = byUser[scope];
  const keys = Object.keys(byScope).sort().reverse();
  const limit = scope === 'day' ? 14 : scope === 'week' ? 10 : 18; // days/weeks/months
  const keep = new Set([keepKey, ...keys.slice(0, limit-1)]);
  for(const k of keys){ if(!keep.has(k)) delete byScope[k]; }
  byUser[scope] = byScope;
  root[userId] = byUser;
  _writeTodoRoot(root);
}
...
async function loadTodosFromSupabase(userId, scope, scopeKey) {
  setSyncStatus('syncing');
  try {
...
async function addTodoToSupabase(todo, scope, scopeKey) {
  if (!currentUser) return;
  setSyncStatus('syncing');
  try {
    const { data, error } = await db.from('todos').insert({
      user_id: currentUser.id, todo_id: todo.id,
      text: todo.text, done: todo.done, scope, scope_key: scopeKey
    }).select().single();
    if (error) throw error;
    todo._dbId = data.id;
    setSyncStatus('synced');
  } catch (e) { console.error('Todo add error:', e); setSyncStatus('error'); }
}

async function updateTodoInSupabase(todo) {
  if (!currentUser || !todo._dbId) return;
  setSyncStatus('syncing');
  try {
    const { error } = await db.from('todos').update({ done: todo.done }).eq('id', todo._dbId);
    if (error) throw error;
    setSyncStatus('synced');
  } catch (e) { console.error('Todo update error:', e); setSyncStatus('error'); }
}
...
async function deleteTodoFromSupabase(todoId) {
  ...
  setSyncStatus('syncing');
  try {
    const { error } = await db.from('todos').delete()
      .eq('user_id', currentUser.id).eq('todo_id', todoId);
    if (error) throw error;
    setSyncStatus('synced');
  } catch (e) { console.error('Todo delete error:', e); setSyncStatus('error'); }
}

let _todoState = null;

async function _loadActiveTodosAsync() {
  if (!_todoState) return;
  const { userId, scope } = _todoState;
  const key = _activeKey();
  // 今日スコープはテンプレート方式（チェックが毎日リセット）
  if (scope === 'day') {
    _todoState.todos = _getDayTodosForDate(userId, key);
    return;
  }
  if (userId !== 'anon') {
    const result = await loadTodosFromSupabase(userId, scope, key);
    if (result !== null) { _todoState.todos = result; return; }
  }
  _todoState.todos = loadTodos(userId, scope, key);
}

async function initTodoPanel(user){
  const titleEl = document.getElementById('js-todo-title');
  const listEl = document.getElementById('js-todo-list');
  const formEl = document.getElementById('js-todo-add-form');
  const inputEl = document.getElementById('js-todo-input');
  const progressEl = document.getElementById('js-todo-progress');
  const hintEl = document.getElementById('js-todo-hint');
  const graphEl = document.getElementById('js-todo-graph');

  const btnDay = document.getElementById('js-todo-scope-day');
  const btnWeek = document.getElementById('js-todo-scope-week');
  const btnMonth = document.getElementById('js-todo-scope-month');

  if(!listEl || !formEl || !inputEl || !progressEl || !btnDay || !btnWeek || !btnMonth) return;

  const userId = user?.id || 'anon';

  _todoState = {
    userId, scope: 'day',
    keys: { day: getDayKeyFromSelected(), week: getWeekKeyFromSelected(), month: getMonthKeyFromContext() },
    todos: [],
    els: { titleEl, listEl, formEl, inputEl, progressEl, hintEl, graphEl, btnDay, btnWeek, btnMonth }
  };

  await _loadActiveTodosAsync();
  _setTodoHeader();
  _setTodoActiveTab();
  renderTodoPanel();
  ...
  formEl.addEventListener('submit', async (e)=>{
    ...
    const todo = { id, text, done:false, createdAt: Date.now() };
    inputEl.value = '';
    if (_todoState.scope === 'day') {
      // テンプレートに追加（全日共通）
      const tmpl = _readDailyTmpl(_todoState.userId);
      tmpl.unshift(todo);
      _writeDailyTmpl(_todoState.userId, tmpl);
      _todoState.todos = _getDayTodosForDate(_todoState.userId, _activeKey());
    } else {
      _todoState.todos.unshift(todo);
      _persistActiveTodos();
      await addTodoToSupabase(todo, _todoState.scope, _activeKey());
    }
    renderTodoPanel();
  });
  ...
  listEl.addEventListener('click', async (e)=>{
    ...
    if(e.target.closest?.('.weekly-todo-del')){
      if (_todoState.scope === 'day') {
        // 日テンプレートから削除
        const tmpl = _readDailyTmpl(_todoState.userId).filter(t => t.id !== id);
        _writeDailyTmpl(_todoState.userId, tmpl);
        _todoState.todos = _getDayTodosForDate(_todoState.userId, _activeKey());
      } else {
        _todoState.todos = _todoState.todos.filter(t => t.id !== id);
        _persistActiveTodos();
    ...
    const t = _todoState.todos.find(t => t.id === id);
    if(!t) return;
    t.done = !t.done;
    if (_todoState.scope === 'day') {
      // 今日の達成状態を履歴に保存
      _saveDayCompletion(_todoState.userId, _activeKey(), _todoState.todos);
    } else {
      _persistActiveTodos();
      await updateTodoInSupabase(t);
    }
    renderTodoPanel();
  });
  ...
  btnDay.addEventListener('click',  ()=>onScope('day'));
  btnWeek.addEventListener('click', ()=>onScope('week'));
  btnMonth.addEventListener('click',()=>onScope('month'));

  // Auto refresh keys (midnight/week/month boundary, or navigation)
  setInterval(()=>{
    if(!_todoState) return;
    const prev = JSON.stringify(_todoState.keys);
    _syncTodoKeysFromUI();
    const now = JSON.stringify(_todoState.keys);
    if (prev !== now) {
      _loadActiveTodosAsync().then(()=>{ _setTodoHeader(); renderTodoPanel(); });
    }
  }, 60_000);
}

function _syncTodoKeysFromUI(){
  if(!_todoState) return;
  _todoState.keys.day = getDayKeyFromSelected();
  _todoState.keys.week = getWeekKeyFromSelected();
  _todoState.keys.month = getMonthKeyFromContext();
}

function _activeKey(){
  return _todoState?.keys?.[_todoState.scope];
}
...
function _persistActiveTodos(){
  if(!_todoState) return;
  const key = _activeKey();
  saveTodos(_todoState.userId, _todoState.scope, key, _todoState.todos);
}

function _setTodoActiveTab(){
  if(!_todoState) return;
  const { btnDay, btnWeek, btnMonth } = _todoState.els;
  btnDay.classList.toggle('is-active', _todoState.scope === 'day');
  btnWeek.classList.toggle('is-active', _todoState.scope === 'week');
  btnMonth.classList.toggle('is-active', _todoState.scope === 'month');
}

function _setTodoHeader(){
  if(!_todoState) return;
  const { titleEl, hintEl } = _todoState.els;
  const k = _activeKey();
  if(_todoState.scope === 'day'){
    const [y,m,d] = k.split('-').map(Number);
    const label = formatYMDSlash(new Date(y, m-1, d));
    if(titleEl) titleEl.textContent = `今日のTODO (${label})`;
    if(hintEl) hintEl.textContent = '日付が変わるとチェックが自動でリセットされます';
  }else if(_todoState.scope === 'week'){
    if(titleEl) titleEl.textContent = `今週のTODO (${getWeekRangeLabel(k)})`;
    if(hintEl) hintEl.textContent = '月曜はじまり。週が変わるとチェックが自動でリセットされます';
  }else{
    // month
    const [y,m] = k.split('-').map(Number);
    if(titleEl) titleEl.textContent = `今月のTODO (${y}/${_pad2(m)})`;
    if(hintEl) hintEl.textContent = '月が変わるとチェックが自動でリセットされます';
  }
}

// 直近 N 日の達成率棒グラフ HTML を返す
function _renderTodoGraph(userId, days = 28) {
  const hist = _readDailyHist(userId);
  const today = new Date();
  let html = '<div class="todo-graph">';
  for (let i = days - 1; i >= 0; i--) {
    ...
    const pct = hasData ? Math.round(rec.done / rec.total * 100) : 0;
    const cls = !hasData ? '' : pct >= 100 ? 'is-full' : pct >= 50 ? 'is-half' : 'is-low';
    const isToday = i === 0;
    html +=
      `<div class="todo-graph-col${isToday ? ' is-today' : ''}" title="${dk}${hasData ? ' '+pct+'% ('+rec.done+'/'+rec.total+')' : ''}">` +
      `<div class="todo-graph-bar-wrap"><div class="todo-graph-bar ${cls}" style="height:${pct}%"></div></div>` +
      `<div class="todo-graph-day">${d.getDate()}</div>` +
      `</div>`;
  }
  html += '</div>';
  return html;
...
function renderTodoPanel(){
  if(!_todoState) return;
  const { todos, els, userId, scope } = _todoState;
  const done = todos.filter(t => t.done).length;
  const total = todos.length;

  const progressText = total === 0 ? '0/0' : `${done}/${total}`;
  els.progressEl.textContent = (total > 0 && done === total) ? `🎉 ${progressText}` : progressText;

  // 今日スコープ：完了状態を履歴に随時保存 + グラフを描画
  const graphEl = els.graphEl;
  if (scope === 'day') {
    if (total > 0) _saveDayCompletion(userId, _activeKey(), todos);
    if (graphEl) graphEl.innerHTML = _renderTodoGraph(userId);
  } else {
    if (graphEl) graphEl.innerHTML = '';
  }

  els.listEl.innerHTML = todos.map(t => `
    <div class="weekly-todo-item ${t.done ? 'done':''}" data-todo-id="${t.id}">
      <div class="weekly-todo-check" aria-hidden="true"></div>
      <div class="weekly-todo-text" title="${escapeHtml(t.text)}">${escapeHtml(t.text)}</div>
      <button class="weekly-todo-del" type="button" title="削除" aria-label="削除">✕</button>
    </div>
  `).join('');

  if(total === 0){
    const msg = scope === 'day'
      ? '毎日繰り返すTODOを追加すると、日付が変わるたびにチェックが自動リセットされます'
      : scope === 'week'
        ? '今週やりたいことを追加して、チェックで達成を管理できます'
        : '今月やりたいことを追加して、チェックで達成を管理できます';
    els.listEl.innerHTML = `
      <div style="opacity:.7;font-size:12px;padding:8px 2px;">
        ${msg}
      </div>
    `;
  }
...
function initBottomPanelTabs() {
  ...
  tabs.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.getElementById('js-bp-todo').style.display = target === 'todo' ? '' : 'none';
