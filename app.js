...
async function initTodoPanel(user){
  ...
  _todoState = {
    ...
    todos: [],
    els: { titleEl, listEl, formEl, inputEl, progressEl, hintEl, btnDay, btnWeek, btnMonth }
  };
...
function renderTodoPanel(){
  ...
  if(total === 0){
    const msg = _todoState.scope === 'day'
      ? '今日やりたいことを追加して、チェックで達成を管理できます'
      : _todoState.scope === 'week'
...
function renderBudgetPanel() {
  ...
  // ── Merge manual entries + shift entries ──
  ...
  if (els.summaryEl) {
    ...
    if (topCats.length) {
      ...
      for (var ci = 0; ci < topCats.length; ci++) {
        ...
        catBreakdown +=
          ...
          '<span class="budget-cat-pct">' + pct + '%</span></div>';
      }
      catBreakdown += '</div>';
    }
    ...
    els.summaryEl.innerHTML =
      ...
      '<div class="budget-summary-card is-income"><div class="budget-summary-label">\u53CE\u5165</div><div class="budget-summary-value">' + fmtYen(totalIncome) + '</div></div>' +
      '<div class="budget-summary-card is-expense"><div class="budget-summary-label">\u652F\u51FA</div><div class="budget-summary-value">' + fmtYen(totalExpense) + '</div></div>' +
      '<div class="budget-summary-card is-balance ' + (balance >= 0 ? 'is-positive' : 'is-negative') + '"><div class="budget-summary-label">\u53CE\u652F</div><div class="budget-summary-value">' + (balance >= 0 ? '+' : '') + fmtYen(balance) + '</div></div>' +
  ...
  for (var gi = 0; gi < gKeys.length; gi++) {
    ...
    for (var ii = 0; ii < gItems.length; ii++) {
      ...
      html += '<div class="budget-entry ' + (isInc ? 'is-income' : 'is-expense') + (isShiftEntry ? ' is-auto' : '') + '" data-budget-id="' + gen.id + '">' +
        '<span class="budget-entry-icon' + (isShiftEntry ? ' is-shift-icon' : '') + '">' + gIcon + '</span>' +
        '<div class="budget-entry-body"><span class="budget-entry-cat">' + gCatName + (isShiftEntry ? '<span class="budget-auto-badge">\u81EA\u52D5</span>' : '') + '</span>' +
        ...
        (isShiftEntry ? '' : '<button class="budget-entry-del" type="button" title="\u524A\u9664" aria-label="\u524A\u9664">\u2715</button>') +
        '</div>';
    }
    html += '</div>';
  }
