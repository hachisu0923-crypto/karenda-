...
document.addEventListener('keydown',e=>{
  ...
  if (e.key==='Escape'){
    closeOverlay('js-day-overlay');closeOverlay('js-cat-overlay');closeEditModal();closeColorPopup();closeOverlay('js-receipt-overlay');closeOverlay('js-apikey-overlay');
...
function initTodoPanel(user){
  ...
  _setTodoHeader();
  _setTodoActiveTab();
  pruneTodos(userId, _todoState.scope, _todoState.keys[_todoState.scope]);
  renderTodoPanel();
  ...
  formEl.addEventListener('submit', (e)=>{
    ...
    _todoState.todos.unshift({ id, text, done:false, createdAt: Date.now() });
    inputEl.value = '';
    _persistActiveTodos();
    renderTodoPanel();
  });
  ...
  const onScope = (next)=>{
    ...
    _setTodoHeader();
    _setTodoActiveTab();
    pruneTodos(_todoState.userId, _todoState.scope, _todoState.keys[_todoState.scope]);
    renderTodoPanel();
  };
  ...
  setInterval(()=>{
    ...
    if (prev !== now) {
      // if active scope's key changed, reload
      _loadActiveTodos();
      _setTodoHeader();
      pruneTodos(_todoState.userId, _todoState.scope, _todoState.keys[_todoState.scope]);
      renderTodoPanel();
...
function getBudgetCat(catId) {
  return BUDGET_EXPENSE_CATS.find(c => c.id === catId)
      || BUDGET_INCOME_CATS.find(c => c.id === catId)
...
function _updateBudgetCatOptions() {
  ...
  const cats = type === 'income' ? BUDGET_INCOME_CATS : BUDGET_EXPENSE_CATS;
...
const RECEIPT_APIKEY_STORAGE = 'cal_anthropic_key';
...
document.getElementById('js-receipt-analyze').addEventListener('click', async function() {
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: 'text',
              text: 'このレシートの内容を解析して、以下のJSON形式で返してください。JSON以外は絶対に出力しないでください。\n\n```\n{\n  "store": "店舗名",\n  "date": "YYYY-MM-DD",\n  "items": [\n    { "name": "商品名", "amount": 金額(整数), "category": "カテゴリID" }\n  ],\n  "total": 合計金額(整数)\n}\n```\n\ncategoryは以下から最も適切なものを選んでください:\n- food: 食費（食料品・飲料・レストラン）\n- transport: 交通費\n- housing: 住居費\n- utility: 光熱費\n- telecom: 通信費\n- entertain: 娯楽費\n- medical: 医療費\n- clothing: 衣服費\n- daily: 日用品（洗剤・トイレットペーパーなど）\n- education: 教育費\n- other_exp: その他\n\n日付が読み取れない場合は "date": null としてください。\n金額は税込の数値（整数）にしてください。\n商品が多い場合は主要なものをまとめてください。'
            }
          ]
        }]
      })
    ...
    if (errMsg.includes('401')) {
      errMsg = 'APIキーが無効です。設定を確認してください。';
    } else if (errMsg.includes('429')) {
      errMsg = 'APIのレート制限に達しました。少し待ってから再試行してください。';
    ...
    row.innerHTML =
      ...
      '<div class="receipt-item-body">' +
        ...
        '<select class="receipt-item-cat-select" data-field="category">' +
          BUDGET_EXPENSE_CATS.map(function(c) {
