...
let isDark         = loadLocalJSON('cal_dark') ?? true;
let activeTab      = 'event';
...
/* ============================================================
   BLENDER UI — Icon Sidebar + Panel Resize
   ============================================================ */

(function initBlenderUI() {

  /* ── Icon sidebar tab switching ── */
  const iconSidebar = document.getElementById('js-icon-sidebar');
  const sidebar     = document.getElementById('js-sidebar');
  if (!iconSidebar || !sidebar) return;

  // Panel sections to show/hide in sidebar
  const panelSections = {
    calendar: ['js-sidebar-mini-cal', 'js-sidebar-cats'],
    category: ['js-sidebar-cats'],
    salary:   ['js-sidebar-salary'],
    user:     ['js-sidebar-user'],
  };

  // Add IDs to sidebar sections for panel switching
  const miniCalEl   = sidebar.querySelector('.mini-cal');
  const catLabelEl  = sidebar.querySelector('.sidebar-section-label');
  const catListEl   = sidebar.querySelector('.category-list');
  const catBtnEl    = sidebar.querySelector('#js-open-cat-editor');
  const salaryLabelEl = sidebar.querySelectorAll('.sidebar-section-label')[1];
  const salaryEl    = sidebar.querySelector('#js-salary-summary');
  const dividers    = sidebar.querySelectorAll('.sidebar-divider');
  const userEl      = sidebar.querySelector('.user-row');
  ...
  function showSidebarPanel(panel) {
    ...
    const allSections = [miniCalEl, catLabelEl, catListEl, catBtnEl,
                         salaryLabelEl, salaryEl, userEl];
    // Show/hide entire sidebar
    if (panel === 'user') {
      // Show only user row, hide rest
      allSections.forEach(el => el && (el.style.display = 'none'));
      if (userEl) userEl.style.display = 'flex';
      dividers.forEach(d => d.style.display = 'none');
    } else if (panel === 'salary') {
      allSections.forEach(el => el && (el.style.display = 'none'));
      if (salaryLabelEl) salaryLabelEl.style.display = 'block';
      if (salaryEl) salaryEl.style.display = '';
      dividers.forEach(d => d.style.display = 'none');
    } else if (panel === 'category') {
      allSections.forEach(el => el && (el.style.display = 'none'));
      if (catLabelEl) catLabelEl.style.display = 'block';
      if (catListEl)  catListEl.style.display = '';
      if (catBtnEl)   catBtnEl.style.display = '';
      dividers.forEach(d => d.style.display = 'none');
    } else {
      // calendar — show all
      allSections.forEach(el => el && (el.style.display = ''));
      dividers.forEach(d => d.style.display = '');
    }

    // Update status bar
    const statusLabels = { calendar: 'カレンダー', category: 'カテゴリ', salary: '給料', user: 'ユーザー' };
    setStatusLeft(statusLabels[panel] || '');
  }

  iconSidebar.querySelectorAll('.icon-tab[data-panel]').forEach(btn => {
    btn.addEventListener('click', () => {
      const panel = btn.dataset.panel;

      // Toggle sidebar visibility if same panel clicked
      if (panel === currentPanel && !sidebar.classList.contains('is-hidden')) {
        sidebar.classList.add('is-hidden');
        iconSidebar.querySelectorAll('.icon-tab').forEach(b => b.classList.remove('is-active'));
        setStatusLeft('');
        return;
      }
  ...
  /* ── Panel resize handle ── */
  const handle      = document.getElementById('js-panel-handle');
  const viewport    = handle ? handle.previousElementSibling : null;
  const bottomPanel = handle ? handle.nextElementSibling : null;
  const mainEl      = handle ? handle.closest('.main') : null;

  if (handle && bottomPanel && mainEl) {
    let dragging = false;
    let startY   = 0;
    let startH   = 0;
    ...
    document.addEventListener('mousemove', e => {
      if (!dragging) return;
      const delta = startY - e.clientY; // drag up = increase bottom panel
  ...
  /* ── Status bar helpers ── */
  window.setStatusLeft  = (txt) => {
    const el = document.getElementById('js-status-left');
    if (el) el.textContent = txt || 'Ready';
  };
  window.setStatusRight = (txt) => {
    const el = document.getElementById('js-status-right');
    if (el) el.textContent = txt || 'My Calendar';
  };

})();

/* ============================================================
   SWIPE NAVIGATION — 左右スワイプで前後の月/週/日に移動
   ============================================================ */
(function initSwipeNavigation() {
  const SWIPE_MIN_X = 50;  // 横方向の最小スワイプ距離 (px)
  const SWIPE_MAX_Y = 80;  // 縦方向の最大許容距離 (px) — 縦スクロールと区別
  ...
  /* スワイプ対象エリア: カレンダービューポート */
  const target = document.querySelector('.calendar-viewport') ||
  ...
  target.addEventListener('touchend', (e) => {
    ...
    // 横移動が最小距離以上、縦移動が許容範囲内、800ms 以内
    if (Math.abs(dx) < SWIPE_MIN_X) return;
    if (Math.abs(dy) > SWIPE_MAX_Y) return;
    if (dt > 800) return;

    const goNext = dx < 0;  // 左スワイプ → 次へ、右スワイプ → 前へ

    if (currentView === 'month') {
      curDate.setMonth(curDate.getMonth() + (goNext ? 1 : -1));
      renderAll();
    } else if (currentView === 'week') {
      wvDate.setDate(wvDate.getDate() + (goNext ? 7 : -7));
      renderWeekView();
    } else if (currentView === 'day') {
      dvDate.setDate(dvDate.getDate() + (goNext ? 1 : -1));
      renderDayView();
    }
